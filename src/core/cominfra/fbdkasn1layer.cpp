...
const std::set<CIEC_ANY::EDataTypeID> CFBDKASN1ComLayer::scmSimpleEncodableDataTypes{
    ...
    CIEC_ANY::e_UINT,  CIEC_ANY::e_UDINT, CIEC_ANY::e_ULINT, CIEC_ANY::e_BYTE,         CIEC_ANY::e_WORD,
    CIEC_ANY::e_DWORD, CIEC_ANY::e_LWORD, CIEC_ANY::e_DATE,  CIEC_ANY::e_TIME_OF_DAY,  CIEC_ANY::e_DATE_AND_TIME,
    CIEC_ANY::e_CHAR,  CIEC_ANY::e_WCHAR, CIEC_ANY::e_LDATE, CIEC_ANY::e_LTIME_OF_DAY, CIEC_ANY::e_LDATE_AND_TIME,
    CIEC_ANY::e_REAL,  CIEC_ANY::e_LREAL};
...
EComResponse CFBDKASN1ComLayer::recvData(const void *paData, unsigned int paSize) {
  ...
  if (mFb != nullptr) {
    ...
    while (usedBufferSize != 0) {
      ...
      // required if e.g. the first element is an bool and the second element is "fragmented" - otherwise the resize in
      ...
      if (mDOPos >= unNumRD) {
        ...
        // FIXME if at this point data is left it is from a second run. Better first deliver this packet and afterwards
        // handle second run
      }
    }
  }
...
int CFBDKASN1ComLayer::serializeDataPointArray(TForteByte *paBytes,
...
void CFBDKASN1ComLayer::serializeTag(TForteByte *paBytes, const CIEC_ANY &paCIECData) {
  ...
  switch (eDataType) {
    ...
    default: paBytes[0] = csmDataTags[eDataType][0]; break;
  }
}
  ...
  if ((CIEC_ANY::e_BOOL < eDataType && eDataType <= CIEC_ANY::e_LTIME) || (CIEC_ANY::e_REAL == eDataType) ||
    ...
    switch (eDataType) {
      ...
      case CIEC_ANY::e_STRUCT:
        ...
        break;
#ifdef FORTE_SUPPORT_CUSTOM_SERIALIZABLE_DATATYPES
      case CIEC_ANY::e_External: nRetVal = paCIECData.serializeCustomType(paBytes, paStreamSize); break;
#endif /* FORTE_SUPPORT_CUSTOM_SERIALIZABLE_DATATYPES */
...
int CFBDKASN1ComLayer::serializeValueSimpleDataType(TForteByte *paBytes,
                                                    int paStreamSize,
                                                    const CIEC_ANY &paDataPoint) {
...
bool CFBDKASN1ComLayer::deserializeDataPointArray(const TForteByte *paBytes,
...
bool CFBDKASN1ComLayer::deserializeTag(const TForteByte paByte, CIEC_ANY &paCIECData) {
  ...
  switch (eDataType) {
    case CIEC_ANY::e_ANY: bRetVal = deserializeTag(paByte, paCIECData.unwrap()); break;
    case CIEC_ANY::e_BOOL:
      bRetVal = (((+e_APPLICATION + +e_PRIMITIVE) == paByte) ||
                 ((+e_APPLICATION + +e_PRIMITIVE + +CIEC_ANY::e_BOOL) == paByte));
      if (bRetVal) {
    ...
    default: bRetVal = (csmDataTags[eDataType][0] == paByte); break;
  }
  return bRetVal;
}
...
int CFBDKASN1ComLayer::deserializeValue(const TForteByte *paBytes, int paStreamSize, CIEC_ANY &paCIECData) {
  ...
  if (scmSimpleEncodableDataTypes.find(eDataType) !=
      scmSimpleEncodableDataTypes.end()) { // e_SINT to e_DATE_AND_TIME can be handled the same way
    nRetVal = deserializeValueSimpleDataType(paBytes, paStreamSize, paCIECData);
  } else {
    switch (eDataType) {
      case CIEC_ANY::e_ANY: nRetVal = deserializeValue(paBytes, paStreamSize, paCIECData.unwrap()); break;
      case CIEC_ANY::e_BOOL:
...
#ifdef FORTE_SUPPORT_CUSTOM_SERIALIZABLE_DATATYPES
      case CIEC_ANY::e_External: nRetVal = paCIECData.deserializeCustomType(paBytes, paStreamSize); break;
#endif /* FORTE_SUPPORT_CUSTOM_SERIALIZABLE_DATATYPES */
      default: break;
    }
  }
...
int CFBDKASN1ComLayer::deserializeValueSimpleDataType(const TForteByte *paBytes,
                                                      int paStreamSize,
                                                      CIEC_ANY &paIECData) {
  int nRetVal = -1;
...
int CFBDKASN1ComLayer::deserializeValueTime(const TForteByte *paBytes, int paStreamSize, CIEC_TIME &paIECData) {
  int nRetVal = -1;
  if (paStreamSize >= 8) {
    TForteInt64 timeInMicroSeconds = ((((TForteInt64) paBytes[0]) << 56) & 0xFF00000000000000LL) +
                                     ((((TForteInt64) paBytes[1]) << 48) & 0x00FF000000000000LL) +
                                     ((((TForteInt64) paBytes[2]) << 40) & 0x0000FF0000000000LL) +
                                     ((((TForteInt64) paBytes[3]) << 32) & 0x000000FF00000000LL) +
                                     ((((TForteInt64) paBytes[4]) << 24) & 0x00000000FF000000) +
                                     ((((TForteInt64) paBytes[5]) << 16) & 0x0000000000FF0000) +
                                     ((((TForteInt64) paBytes[6]) << 8) & 0x000000000000FF00) +
                                     (((TForteInt64) paBytes[7]) & 0x00000000000000FF);
...
int CFBDKASN1ComLayer::deserializeValueWString(const TForteByte *paBytes, int paStreamSize, CIEC_WSTRING &paIECData) {
  int nRetVal = -1;
  if (paStreamSize >= 2) {
    TForteUInt16 unStringSize = static_cast<TForteUInt16>(((static_cast<TForteUInt16>(paBytes[0]) << 8) & 0xFF00) +
                                                          (static_cast<TForteUInt16>(paBytes[1]) & 0x00FF));
...
int CFBDKASN1ComLayer::deserializeValueString(const TForteByte *paBytes, int paStreamSize, CIEC_STRING &paIECData) {
  int nRetVal = -1;
  if (paStreamSize >= 2) {
    TForteUInt16 unStringSize =
        static_cast<TForteUInt16>(((((TForteUInt16) paBytes[0]) << 8) & 0xFF00) + ((TForteUInt16) paBytes[1] & 0x00FF));
...
int CFBDKASN1ComLayer::deserializeValueBoolArray(const TForteByte *paBytes,
                                                 int paStreamSize,
                                                 CIEC_ARRAY &paArray,
...
size_t CFBDKASN1ComLayer::getRequiredSerializationSize(const CIEC_ANY &paCIECData) {
  ...
  switch (paCIECData.getDataTypeID()) {
    ...
    default: unRetVal += csmDataTags[paCIECData.getDataTypeID()][1]; break;
  }
  return unRetVal;
}
