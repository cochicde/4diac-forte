...
void CInternalTracer::traceInstanceData(const char *const paTypeName, const char *const paInstanceName,
  ...
  mEvents.emplace_back("instanceData", std::make_unique<FBInstanceDataPayload>(paTypeName, paInstanceName, inputs, outputs, internal, internalFBs), getNanoSecondsMonotonic());
}
...
void CInternalTracer::traceInputData(const char *const paTypeName, const char *const paInstanceName,
  ...
  mEvents.emplace_back("inputData", std::make_unique<FBDataPayload>(paTypeName, paInstanceName, paDataId, paValue), getNanoSecondsMonotonic());
}
...
void CInternalTracer::traceOutputData(const char *const paTypeName, const char *const paInstanceName, const uint64_t paDataId,
  ...
  mEvents.emplace_back("outputData", std::make_unique<FBDataPayload>(paTypeName, paInstanceName, paDataId, paValue), getNanoSecondsMonotonic());
}
